...
static int rx_socket(picoserver_socket_t *client_socket);
static void tx_queue_handle(void);
static int rx_queue_handle(void);
...
static void socket_cb(uint16_t ev, struct pico_socket *s)
{
    ...
    if (client_socket->async_transport) {
        if (ev & PICO_SOCK_EV_RD) {
            ev &= ~PICO_SOCK_EV_RD;
            int count = rx_queue_handle();
            count += rx_socket(client_socket);
            if (count == 0) {
                printf("empty\n");
            }
        }
...
static void tx_socket(picoserver_socket_t *client_socket)
{
    ...
    while (client_socket->async_transport->tx_pending_queue) {
        ...
        if (client_socket->protocol == PICO_PROTO_UDP) {
            ret = pico_socket_sendto(client_socket->socket, msg->buf+msg->done_len, msg->total_len-msg->done_len, &msg->src_addr, msg->remote_port);
        } else {
...
static void tx_queue_handle(void)
{
    ...
    while (1) {
        ...
        ZF_LOGF_IF((msg->total_len > 1400) || (msg->total_len == 0), "bad msg len in tx %d", msg->total_len);
...
static int rx_socket(picoserver_socket_t *client_socket)
{
    ...
    while (client_socket->async_transport->rx_pending_queue) {
        ...
        if (client_socket->protocol == PICO_PROTO_UDP) {
            ret = pico_socket_recvfrom(client_socket->socket, msg->buf+msg->done_len, msg->total_len-msg->done_len, &msg->src_addr, &msg->remote_port);
        } else {
...
static int rx_queue_handle(void)
{
    ...
    while(1) {
        ...
        tx_msg_t *msg = DECODE_DMA_ADDRESS(buf);
        ZF_LOGF_IF(msg == NULL, "msg is null");
        ZF_LOGF_IF((msg->total_len > 1400) || (msg->total_len == 0), "bad msg len in rx %d", msg->total_len);
